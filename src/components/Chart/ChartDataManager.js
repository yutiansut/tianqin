...
class ChartDM {
  ...
  resetChartParams () {
    ...
    this._barNumbers = Math.floor(this._width / this._barWidth)
    this._barPadding = Math.max(BAR_PADDING_MIN, Math.round(this._barWidth * BAR_PADDING_RATE))
  ...
  get quote () {
    ...
    return this._symbol ? this.tqsdk.get({ name: 'quote', symbol: this._symbol }) : null
  }

  get klines () {
    // return (this._symbol && this._duration) ? this.tqsdk.get_klines(this._symbol, this._duration) : null
    return (this._symbol && this._duration) ? this.tqsdk.get({ name: 'klines', symbol: this._symbol, duration: this._duration })
      : null
  }
